...
f=open('possibleTransients'+'_sw'+str(sys.argv[4])+'_ip'+str(sys.argv[5])+'.txt','r')
...
#            print(obj)
            try:
                helpplot(dataDirectory+j)
                print("Plotting "+j)
                pyplot.savefig(folderForSaving+'/'+obj+'/'+j+'.png')
                pyplot.clf()
            except:
                print('Something wrong with file '+str(dataDirectory+j)+'.  Probably only has upper limits.  Could not plot data')
